...
authRouter.post(
  ...
  try {
    ...
    await OtpChallengeModel.deleteMany({ phone: body.phone, consumedAt: null });
    ...
    // Never log OTP. Dev/test only: return in body for local UX.
    const payload: {
      ok: true;
      phone: string;
      expiresInSec: number;
...
authRouter.post(
  ...
  try {
    ...
    const ok = safeEqual(challenge.codeHash, hashValue(body.otp));
    if (!ok) {
